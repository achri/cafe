...
class Dynatree {
	...
	function generate_kategori_tree($kat_id,$kat_level=array('1','2','3'),$kat_tipe=false,$kat_jenis=false) {
		$qlevel = self::$CI->dynatree_model->set_kategori($kat_id,$kat_level,$kat_tipe);
		...
		foreach($qlevel->result() as $rows):
			...
			if ($kat_jenis != false)
				$json .= ',"jenis": "'.$kat_jenis.'"';
			
			$json .= ',"lvl": "'.$rows->kat_level.'"';
			
			$qlevel2 = self::$CI->dynatree_model->set_kategori($rows->kat_id,$kat_level,$kat_tipe);

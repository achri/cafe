...
class Tbl_order extends CI_Model {
	...
	function cek_meja_order($no_meja) {
		...
		where ord.no_meja = $no_meja and ord.status in (0,1,2,3) and ord.bill_id = 0";
	...
	function ubah_order($where,$data) {
		if (is_array($where)):
			foreach ($where as $field=>$value):
				$this->db->where($field,$value);
			endforeach;
		...
		return $this->db->update('order_menu',$data);
	}
